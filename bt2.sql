...
call AddInventory(10, -20);
-- lệnh update hiện tại không kiểm tra giá trị âm của p_quantity
-- do đó nhân viện nhập -20 thì phép tính stock_quantity + (-20) sẽ làm số lượng hàng tồn kho bị giảm
-- gây mất hàng trong kho
...
begin
	if p_quantity < 0 then
		select 'Lỗi số lượng không được nhỏ hơn 0' as error_quantity;
	else 
		UPDATE Inventory
		SET stock_quantity = stock_quantity + p_quantity
		WHERE item_id = p_item_id;
